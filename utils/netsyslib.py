...
class Node:
    ...
    def get_next_step(self, ip):
        for network, values in self.ip_table.items():
            if network.is_in(netlib.Network(ip,32)):
                return values['next step']
        else:
            raise NetSystemException('No route to net')

    def get_routing_cost(self, ip):
        for network, values in self.ip_table.items():
            if network.is_in(netlib.Network(ip, 32)):
                return values['next step']
        else:
